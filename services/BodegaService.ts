...
export class BodegaService {
  ...
  async obtenerInventarioLocal(id_sucursal: number, filtros?: any) {
    let query = `
      SELECT
        rps.id_reorden                                          AS id_producto_sucursal,
        rps.id_producto,
        p.sku, p.nombre,
        rps.punto_reorden,
        ...
        COALESCE(lotes_costo.costo_promedio_ponderado, 0)      AS costo,
        COALESCE(lotes_costo.total_lotes, 0)                   AS total_lotes,
        (COALESCE(lotes_costo.cantidad_actual, 0) <= rps.punto_reorden) AS requiere_reorden,
        ...
        (SELECT COALESCE(json_agg(json_build_object('sucursal', s2.nombre, 'cantidad', suc_stock.cantidad)), '[]'::json)
         FROM (
           ...
           FROM lote_detalle ld2
           WHERE ld2.id_producto = p.id_producto AND ld2.id_sucursal != $1
             AND ld2.agotado = FALSE AND ld2.activo = TRUE
        ...
        (SELECT COALESCE(json_agg(json_build_object(
            'marca', mv.nombre, 'modelo', mod.nombre,
            'anio_desde', cp.anio_desde, 'anio_hasta', cp.anio_hasta,
      ...
      WHERE rps.id_sucursal = $1 AND rps.activo = TRUE
    `;
    ...
    if (filtros?.id_modelo_vehiculo) {
      query += ` AND EXISTS (
        ...
        WHERE cp.id_producto = p.id_producto
          AND (cp.es_universal = true OR cp.id_modelo = $${paramIndex})
      )`;
  ...
  async obtenerLotesDeProducto(id_producto: number, id_sucursal: number) {
    const query = `
      ...
      WHERE ld.id_producto = $1
        ...
        AND ld.activo      = TRUE
      GROUP BY li.id_lote, li.es_apertura, li.fecha_ingreso
    ...
    try {
      ...
      const resNota = await client.query(
        `INSERT INTO nota_despacho
         ...
         VALUES (
           $1, $2, $3,
           (SELECT id_estado_despacho FROM estado_despacho WHERE nombre = 'pendiente')
         )
      ...
      for (const det of data.detalles) {
        ...
        await client.query(
          `INSERT INTO detalle_despacho (id_despacho, id_producto, cantidad)
           VALUES ($1,$2,$3)`,
          ...
          `INSERT INTO movimiento_inventario
           ...
           VALUES (
             ...
             (SELECT id_tipo_movimiento FROM tipo_movimiento_inventario
              WHERE nombre = 'salida_traslado'),
             $3, $4, $5, 'nota_despacho',
             'Envío de mercadería a otra sucursal'
           )
           RETURNING id_movimiento`,
          [id_reorden, id_usuario, det.cantidad, nueva_cantidad, id_despacho],
        );
    ...
    const query = `
      SELECT
        ...
        (SELECT json_agg(json_build_object(
            'producto', p.nombre, 'sku', p.sku, 'cantidad', dd.cantidad
          ))
      ...
      JOIN estado_despacho  ed  ON nd.id_estado_despacho = ed.id_estado_despacho
      WHERE nd.id_sucursal_destino = $1
        AND ed.nombre              = 'pendiente'
    ...
    try {
      ...
      const notaRes = await client.query(
        `SELECT ed.nombre AS estado, nd.id_sucursal_origen
         FROM nota_despacho nd
         JOIN estado_despacho ed ON nd.id_estado_despacho = ed.id_estado_despacho
         WHERE nd.id_despacho = $1 AND nd.id_sucursal_destino = $2
         FOR UPDATE`,
        [id_despacho, id_sucursal_destino],
      );

      if (!notaRes.rows.length)
        throw new Error(
          "Nota de despacho no encontrada o no pertenece a esta sucursal",
        );
      if (notaRes.rows[0].estado !== "pendiente")
      ...
      await client.query(
        `UPDATE nota_despacho
         SET id_estado_despacho = (
               SELECT id_estado_despacho FROM estado_despacho WHERE nombre = 'recibido'
             ),
             fecha_recepcion = NOW()
         WHERE id_despacho = $1`,
        [id_despacho],
      ...
      for (const det of detallesRes.rows) {
        ...
        const rpsRes = await client.query(
          `INSERT INTO reorden_producto_sucursal (id_producto, id_sucursal)
           ...
           RETURNING id_reorden`,
          [det.id_producto, id_sucursal_destino],
        );
        const id_reorden_destino = rpsRes.rows[0].id_reorden;

        // Stock en destino antes del ingreso
        ...
        const lotesOrigenRes = await client.query(
          `SELECT ld.id_lote, ld.costo_unitario, mi.cantidad AS cantidad_consumida
           ...
           JOIN lote_detalle                  ld  ON mi.id_lote_detalle    = ld.id_lote_detalle
           JOIN tipo_movimiento_inventario    tmi ON mi.id_tipo_movimiento = tmi.id_tipo_movimiento
           WHERE mi.id_referencia    = $1
             AND mi.tabla_referencia = 'nota_despacho'
             AND tmi.nombre          = 'salida_traslado'
             AND ld.id_producto      = $2
             AND ld.id_sucursal      = $3
             AND mi.id_lote_detalle IS NOT NULL
           ORDER BY ld.id_lote ASC`,
          [id_despacho, det.id_producto, id_sucursal_origen],
        );

        if (lotesOrigenRes.rows.length > 0) {
          // Caso normal: hay trazabilidad completa del lote origen
          for (const loteOrigen of lotesOrigenRes.rows) {
            await client.query(
              `INSERT INTO lote_detalle
               ...
               SELECT $1,$2,$3,$4,$5,$6,$6, li.fecha_ingreso
               FROM lote_inventario li WHERE li.id_lote = $1`,
              [
          ...
          const nuevoLoteRes = await client.query(
            `INSERT INTO lote_inventario (es_apertura) VALUES (TRUE) RETURNING id_lote`,
          );
      ...
      const movRes = await client.query(
        `INSERT INTO movimiento_inventario
           (id_reorden, id_usuario, id_tipo_movimiento,
            cantidad, cantidad_resultante, motivo)
         VALUES (
           $1, $2,
           (SELECT id_tipo_movimiento FROM tipo_movimiento_inventario WHERE nombre = $3),
           $4, $5, $6
         )
         RETURNING id_movimiento`,
        [
