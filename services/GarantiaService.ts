...
export class GarantiaService {
  ...
  async resolverGarantia(data: ResolverGarantiaDTO): Promise<void> {
    ...
    try {
      ...
      await client.query(
        `
        UPDATE garantia SET estado = $1, resolucion = $2, id_supervisor_aprueba = $3
        WHERE id_garantia = $4
      `,
        [nuevoEstado, data.resolucion, data.id_supervisor, data.id_garantia],
      );

      // Si es aprobada, le damos un producto nuevo al cliente de nuestro inventario
      if (data.aprobado) {
  ...
  // --- NUEVOS MÉTODOS PARA EL FLUJO FÍSICO DE RETORNOS --- //
