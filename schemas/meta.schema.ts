...
export const asignarMetaSchema = z
  .object({
  ...
  .refine(
    (data) => {
      ...
      const objetivo = data.anio * 12 + data.mes;
      const referencia = anioActual * 12 + mesActual;
      return objetivo >= referencia;
    },
    {
      message:
        "No se pueden asignar metas para meses anteriores al mes en curso",
      path: ["mes"],
    },
  );
